...
def Credits():
    ...
    Cwin.mainloop()
def meshNet():
    def meshNetworkCalculator():
        output_text_widget.delete('1.0', END)
       # result = meshNetworkCalculator(input_text_widget.get())
        output_text_widget.insert(END, mesh_Network_Calculator(input_text_widget.get()))

    meshNetWin = Tk()
    meshNetWin.title('Mesh Network Calculator')

    frame_outer = Frame(meshNetWin, bg='black')
    frame_outer.pack(fill='both', expand=True, padx=5, pady=5)

    col = 'lightgray'  # Define the background color
    frame_inner = Frame(frame_outer, bg=col)
    frame_inner.pack(fill='both', expand=True, padx=2, pady=2)
    ...
    l0 = Label(f1, font=('arial', 25, 'bold'), text='Input Number of Networking Devices:', bg=col)
    input_text_widget = Entry(f2)
    execute_widget = Button(f2, text='Convert', font=('arial', 15, 'bold'), command=meshNetworkCalculator)
    l1 = Label(f3, font=('arial', 25, 'bold'), text='Connections Needed:', bg=col)
    output_text_widget = Text(f4, width=45, height=1)
...
def mesh_Network_Calculator(num_devices):
    try:
        num_devices = int(num_devices)
        if num_devices < 2:
            return "Number of devices should be at least 2."
...
submenu.add_separator()
submenu.add_cascade(label='Mesh Network Calculator', command=meshNet)
submenu.add_separator()
